...
impl QwenCtx {
    ...
    pub fn load(model_dir: &str) -> Option<Self> {
        ...
        let mut info = crate::config::DetectInfo {
            has_enc_layer_18: false,
            lm_head_shape: None,
            embed_tokens_shape: None,
            gate_proj_shape: None,
        };

        // Try with and without "thinker." prefix for detection
        for prefix in &["thinker.", ""] {
            if ms.has_tensor(&format!(
                "{}audio_tower.layers.18.self_attn.q_proj.weight",
                prefix
            )) {
                info.has_enc_layer_18 = true;
            }
            if let Some((_, t)) = ms.find(&format!("{}lm_head.weight", prefix)) {
                info.lm_head_shape = Some(t.shape.as_slice());
            }
            if let Some((_, t)) = ms.find(&format!("{}model.embed_tokens.weight", prefix)) {
                info.embed_tokens_shape = Some(t.shape.as_slice());
            }
            if let Some((_, t)) = ms.find(&format!("{}model.layers.0.mlp.gate_proj.weight", prefix))
            {
                info.gate_proj_shape = Some(t.shape.as_slice());
            }
        }
